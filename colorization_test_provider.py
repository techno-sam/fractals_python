...
def draw(dim: int, pygame, screen, y_x: list[list[tuple[int, float, float]]], display_update: bool = False):
    ...
    for y in range(dim):
        ...
        for x in range(dim):
            data = y_x[y][x]
            screen.set_at((x, y), color_counts(*data, x, y, counts, total))
    if display_update:
        pygame.display.update()
...
def color_counts(iters: int, fx: float, fy: float, x: int, y: int, counts: dict[int, int], total_count: int) -> tuple[int, int, int]:
    if iters == -1:
        return 0, 0, 0
    ...
    #if True:
    #    return _clean(_to_255(colorsys.hls_to_rgb( (((iters/1000)*360)**1.5)/360 , 0.5, 1)))
    try:
        return _clean(_to_255(colorsys.hls_to_rgb(hue**1.5, 0.5, 1)))
    except ValueError:
        return 255, 0, 0
